...
export default function SahaayAuthScreen() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const emailToUse = googleEmail.trim() || 'user.google@gmail.com';
      const nameToUse = googleName.trim() || 'Google User';
      const fallbackUser = {
        ...
        <div className="rounded-[32px] bg-white p-6 sm:p-8 shadow-2xl border border-white/80">
          ...
          {authMethod === 'google' && (
            <div className="space-y-4 text-center py-2">
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-left">
                <div className="flex items-center justify-between mb-1">
                  ...
                  {googleClientId ? (
                    ...
                    <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-blue-50 text-[#2464A4] border border-blue-200">
                      Google OAuth Ready
                    </span>
                  )}
                </div>
              ...
              {/* Official Google GSI Button rendered here when NEXT_PUBLIC_GOOGLE_CLIENT_ID is active */}
              {googleClientId && (
                <div className="space-y-3">
                  <div id="google-btn-container" className="min-h-[44px] flex items-center justify-center w-full" />
                  <p className="text-[11px] text-slate-400">Official Google One Tap &amp; OAuth 2.0 active</p>
                </div>
              )}

              {/* Fallback / Manual Google Sign In when Client ID is not yet provided */}
              {!googleClientId && (
                <div className="space-y-3">
                  <div className="space-y-2 text-left">
                    ...
                    <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 focus-within:border-[#2464A4] focus-within:bg-white transition-all">
                      ...
                      <input
                        ...
                        className="bg-transparent text-xs sm:text-sm font-semibold text-[#0D1C34] focus:outline-none w-full placeholder:text-slate-400"
                      />
                    </div>
                  </div>
                  ...
                  <div className="p-3 rounded-2xl bg-blue-50/70 border border-blue-100 text-[11px] text-[#2464A4] flex items-start gap-2 text-left">
                    <Sparkles className="h-3.5 w-3.5 shrink-0 mt-0.5 text-[#2464A4]" />
                    <span>To activate Google One-Tap popup with your official Google Cloud project, add <code className="bg-blue-100/70 px-1 py-0.5 rounded text-[10px] font-mono">NEXT_PUBLIC_GOOGLE_CLIENT_ID</code> to Vercel.</span>
                  </div>
                </div>
              )}
            </div>
